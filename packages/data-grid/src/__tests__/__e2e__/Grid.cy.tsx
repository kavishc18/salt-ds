...
describe("Grid", () => {
  ...
  it("Scrolls out of grid", () => {
    cy.mount(
      <div style={{height: "300px", display: "grid", rowGap: "20px"}}>
        <LotsOfColumns style={{height: "280px"}} />
        <div>Hello</div>
      </div>
    );
    assertGridReady();
    cy.findByTestId("grid-scrollable").scrollTo(0, 200, {
      easing: "linear",
      duration: 100,
    });
    ...
    // Should have continued scrolling so that grid no longer visible
    getRow(0).should("not.exist");
    getRow(1).should("not.exist");
    getRow(16).should("not.exist");
    getRow(28).should("not.exist");
    getRow(29).should("not.exist");
  });
