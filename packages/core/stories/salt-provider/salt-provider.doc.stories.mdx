...
You can use the `theme` prop to change the default theme (set to `salt-theme`) by passing a theme name. For more details on Theme please go to <a href="?path=/story/theme-about-the-salt-theme--page">Salt Theme</a>
...
Read [`AriaAnnouncer` documentation](/docs/documentation-core-aria-announcer-provider--page) for more details.
